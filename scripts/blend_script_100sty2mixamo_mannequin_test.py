...
if __name__ == '__main__':
    ...
    for file_idx, bvh_file in enumerate(bvh_file_paths):
        ...
        # Create the export path for the BVH file
        ...
        # Import the source motion BVH as before.
        bpy.ops.import_anim.bvh(filepath=bvh_file)
        source_armature = bpy.context.object
        source_armature["retargetted"] = True  # Mark so it's preserved
        ...
        # ----- MODIFIED PART: Import FBX instead of BVH for the target rig -----
        ...
        # Deselect all objects to prepare for export
        bpy.ops.object.select_all(action='DESELECT')

        # First, export the retargeted animation as a BVH file.
        # (The existing `save()` function writes out the BVH file.)
