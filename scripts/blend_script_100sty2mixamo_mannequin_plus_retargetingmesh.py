...
if __name__ == '__main__':
    ...
    for file_idx, bvh_file in enumerate(bvh_file_paths):
        ...
        export_fbx_path = export_path.replace('.bvh', '.fbx')
        ...
        # Import target T-pose and retarget
        ...
        # [T-pose refinement steps remain unchanged]

        bpy.ops.arp.save_pose_rest()
        bpy.ops.object.mode_set(mode='OBJECT')
        bpy.ops.arp.retarget(frame_start=start_frame, frame_end=end_frame)

        # Export retargeted BVH
        bpy.ops.object.select_all(action='DESELECT')
        target_armature.select_set(True)
        bpy.context.view_layer.objects.active = target_armature
        global_matrix = axis_conversion(from_forward="-Z", from_up="Y").to_4x4().inverted()
        save(target_armature, filepath=export_path, frame_start=start_frame, frame_end=end_frame, 
             root_transform_only=True, global_matrix=global_matrix)

        # Export FBX with animated mesh
        if os.path.exists(fbx_model_path):
            ...
            if fbx_armature:
                # Transfer animation from retargeted armature to FBX armature
                if target_armature.animation_data and target_armature.animation_data.action:
                    # Ensure the FBX armature uses the same bone names as the retargeted BVH
                    fbx_armature.animation_data_create()
                    fbx_armature.animation_data.action = target_armature.animation_data.action.copy()
                    
                    # Force Blender to update the animation data
                    bpy.context.view_layer.update()
                    
                    # Select the FBX armature and its mesh for export
                    bpy.ops.object.select_all(action='DESELECT')
        ...
        # Delete source and target armatures AFTER processing FBX
        bpy.ops.object.select_all(action='DESELECT')
        source_armature.select_set(True)
        target_armature.select_set(True)
        bpy.ops.object.delete()

    # [Optional: Import all retargeted BVHs at 0.1 scale if needed]